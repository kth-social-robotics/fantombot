...
def unpickle_from_bucket(bucket, name):
    ...
    """
    s3client = Session().client('s3', endpoint_url='http://127.0.0.1:9000', aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_KEY)
...
def upload_to_s3(data, bucket, name):
    s3 = Session().client('s3', endpoint_url='http://127.0.0.1:9000', aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_KEY)
...
def file_to_s3(bucket_name, from_location, to_location):
    s3 = boto3.resource('s3', endpoint_url='http://127.0.0.1:9000', aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_KEY)
    s3.Bucket(bucket_name).upload_file(from_location, to_location)


def list_files_in_s3_bucket_dir(bucket_name, directory):
    return boto3.resource('s3', endpoint_url='http://127.0.0.1:9000', aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_KEY).Bucket(bucket_name).objects.filter(Prefix=directory)
...
def files_from_s3(bucket_name, directory, to_directory):
    ...
    for object_summary in list(my_bucket.objects.filter(Prefix=directory))[1:]:
        file_from_s3(bucket_name, object_summary.key, to_directory + object_summary.key.split("/")[-1])


def download_dir(bucket, dist, local):
    client = boto3.client('s3', endpoint_url='http://127.0.0.1:9000', aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_KEY)
    resource = boto3.resource('s3', endpoint_url='http://127.0.0.1:9000', aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_KEY)
