...
def test_scoring(list_of_nodes, scoring_function):
    ...
    for i in range(len(list_of_utts)):
        for j in range(i):
            score_mat[i][j] = scoring_function(
                    list_of_utts[i],
                    list_of_utts[j])
    ...
    best_F1, best_threshold, best_precision, best_recall, best_median_F1 = find_thresholds(score_mat, ground_truth)
    result_dict = {'best_F1': best_F1, 'best_threshold': best_threshold, 'total_time': total_time,
            'best_precision': best_precision, 'best_recall': best_recall, 'best_median_F1': best_median_F1}
    print(result_dict)
    return result_dict
...
def find_thresholds(score_mat, ground_truth):
    ...
    return best_mean_F1, best_mean_threshold, best_mean_precision, best_mean_recall, best_median_F1


def generate_features(list_of_nodes, features=None):
    if not features:
        features = gen_feature_dict(WORD_EMBEDDINGS, WORD_CLASS_SCORE, WORD_CLASS_VECTORS, WORD_CLASSES, SENTIMENT, TOPIC, LDA)
